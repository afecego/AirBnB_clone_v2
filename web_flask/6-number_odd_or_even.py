...
def numOddorEven(n):
    """This funtion say if a number is odd or even and return result"""

    if (n % 2 == 0):
        txt = "{:d} is even".format(n)
        return render_template('6-number_odd_or_even.html', number=txt)
    else:
        txt = "{:d} is odd".format(n)
        return render_template('6-number_odd_or_even.html', number=txt)
